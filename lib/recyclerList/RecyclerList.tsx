...
interface RecyclerProps {
  service?: any;
  itemBuilder?: React.ReactNode | React.FC;
  nodeBuilder?: Function | any;
  gridClass?: string;
...
export default React.memo((props: RecyclerProps) => <RecyclerList {...props} />);

class RecyclerList extends React.Component implements IRecyclerList {
  threshold = 200;
  ...
  constructor(props = {} as RecyclerProps) {
    const {
      service,
      itemBuilder,
      nodeBuilder,
  ...
  componentDidMount() {
    this.container = document.getElementById("recycler") as HTMLDivElement;
  ...
  scrollerIndecator.addEventListener("mousedown", (e) => {
    ...
    const mouseUp = () => {
      ...
      recycler.grid.replaceChildren(
        ...recycler.itemsData.slice(recycler.lastItem - recycler.viewedItems, recycler.lastItem).map((item: any) => recycler.buildItem(item))
      );
