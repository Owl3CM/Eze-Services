...
export const ExporterMechanics = {
  ...
  download: async <TItem>(config: ExporterSliceConfig<TItem>, handler: OperationHandler, ctx: ExporterDependencies<TItem>, opts: { type: "csv" | "excel" }) => {
    const run = async () => {
    ...
    // Single flow — noop handler absorbs calls when no status system exists
    try {
      handler.loading({ title: config.operationMessages?.loading ?? "Exporting..." });
      await run();
      handler.idle();
    } catch (e) {
      handler.error({ title: config.operationMessages?.error ?? "Export failed" });
      throw e;
    }
  },
};
