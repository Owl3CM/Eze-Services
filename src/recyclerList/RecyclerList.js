...
class RecyclerListClass extends React.Component {
  threshold = 200
  ...
  constructor({
    ...
    service.setItems = (callBack, clear) => {
      ...
      if (typeof callBack === 'function') {
        ...
        if (!Array.isArray(callBack))
          throw new Error('setItems must be a function or an array')
    ...
    this.buildItem = nodeBuilder
      ? (item) => nodeBuilder(item)
    ...
    this.initGrid = () => {
      this.grid.replaceChildren(
        ...service.items
          .slice(0, this.viewedItems)
          .map((item) => this.buildItem(item))
      )
      this.handleCalculations()
      setTimeout(() => {
        if (
          this.service.canFetch &&
          this.container.offsetHeight - this.grid.offsetHeight > -400
        ) {
          this.service.loadMore().then(() => {
            ...
            this.grid.append(
              ...service.items
                .slice(this.viewedItems, newViewedItems)
                .map((item) => this.buildItem(item))
            )
  ...
  handleCalculations = () => {
    ...
    this.colums = Math.floor(
      this.container.offsetWidth / this.grid.firstChild.offsetWidth
    )
...
const recyclingOnScroll = (target, recycler) => {
  ...
  if (scrollDown) {
    if (target.scrollTop > recycler.centerOfContainer + recycler.threshold) {
      if (recycler.lastItem + recycler.colums < recycler.service.items.length) {
        ...
        if (target.scrollTop < target.scrollHeight)
          setTimeout(() => (target.scrollTop = target.scrollTop - 10), 5)
    ...
    target.scrollTop < recycler.centerOfContainer - recycler.threshold &&
...
const createIndecator = (recycler) => {
  ...
  indecatorContainer.style.backgroundColor =
    recycler.indecatorProps.backgroundColor
  ...
  recycler.updateIndecatorPostion = () => {
    scrollerIndecator.style.top = `${
      ((recycler.lastItem - recycler.viewedItems) /
        recycler.service.items.length) *
      recycler.indecatorAllowdAreay
    }px`
  }
  recycler.updateIndecatorProps = () => {
    ...
    let height =
      (recycler.container.offsetHeight /
        (recycler.service.items.length / recycler.colums)) *
      15
    height =
      height < 20
        ? 20
        : height > recycler.container.offsetHeight
        ? recycler.container.offsetHeight - 20
        : height
  ...
  recycler.onSwipeIndecator = (e) => {
    const lastIndex = Math.floor(
      (e.clientY / recycler.container.offsetHeight) *
        recycler.service.items.length
    )
    if (
      lastIndex <= recycler.viewedItems ||
      lastIndex > recycler.service.items.length
    )
      return
  ...
  scrollerIndecator.addEventListener('mousedown', (e) => {
    ...
    const mouseUp = () => {
      ...
      if (recycler.lastItem - recycler.viewedItems < 0)
        ...
        recycler.lastItem = recycler.service.items.length
      recycler.grid.replaceChildren(
        ...recycler.service.items
          .slice(recycler.lastItem - recycler.viewedItems, recycler.lastItem)
          .map((item) => recycler.buildItem(item))
      )
      recycler.container.scrollTop =
        recycler.container.scrollTop > recycler.centerOfContainer
          ? recycler.container.scrollHeight
          : 0
