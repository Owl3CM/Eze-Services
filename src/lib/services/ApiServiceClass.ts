...
const getErrorRespoinse = (res: any, props: any) => ({ ...props, ...res, statusMessage: ApiService.StatusCodeByMessage[res.status] || "Unknown Error" });
export default class ApiService implements IApiService {
  ...
  constructor({ baseURL, headers, onResponse, onError, interceptor }: Props) {
    const create = async (method: string, api: any) => {
      api[method] = (endpoint: string, body: any) => {
        ...
        let props = { "Content-Type": "application/json", signal: api[abortId].signal, method, headers, body: JSON.stringify(body) || null };
  ...
  static StatusCodeByMessage: { [key: number]: string } = {
